...
export class CodexAppServerClient {
  constructor({
    cwd,
    mcpUrl,
    bearerTokenEnvVar = "REVIEWOPS_DEVICE_TOKEN",
    model = "gpt-5.4",
  ...
  buildArgs() {
    return [
      "app-server",
      "--config",
      `mcp_servers.reviewops.url=${JSON.stringify(this.mcpUrl)}`,
      "--config",
      `mcp_servers.reviewops.bearer_token_env_var=${JSON.stringify(this.bearerTokenEnvVar)}`,
    ];
  }
  ...
  async start() {
    ...
    await this.sendRequest("initialize", {
      clientInfo: {
        name: "reviewops_helper",
        title: "ReviewOps Helper",
        version: "0.1.0",
      },
      capabilities: {
