...
import { ClipboardList, PencilLine, Save, Search, ShieldCheck, ShieldX } from 'lucide-react';
...
export default function ProposalQueue({ filters, proposals }: ProposalQueueProps) {
            ...
            <div className="space-y-8 p-4 md:p-6">
                <section className="rounded-[2rem] bg-[linear-gradient(135deg,_#0f172a_0%,_#111827_55%,_#334155_100%)] p-7 text-white shadow-[0_24px_70px_rgba(15,23,42,0.28)]">
                    ...
                    <h1 className="mt-4 text-3xl font-semibold tracking-tight">
                        Review, edit, and publish the merchant-facing changes Codex prepared.
                    </h1>
                    <div className="mt-6 flex flex-wrap gap-3">
                        {['pending', 'applied', 'rejected', 'all'].map((status) => (
                            <button
                                key={status}
                                type="button"
                                onClick={() =>
                                    router.get(admin.proposals.index().url, { status }, { preserveState: true, preserveScroll: true })
                                }
                ...
                <section className="grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
                    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-slate-900">Queue summary</h2>
                            <Search className="size-5 text-slate-400" />
                        </div>
                        <p className="mt-4 text-sm leading-6 text-slate-600">
                            Pending proposals are the clearest demo surface for trust. Edit them when needed, approve the strongest one, and use the storefront as the before-and-after payoff.
                        </p>
                        <div className="mt-6 space-y-3">
                            {proposals.map((proposal) => (
                                ...
                                >
                                    ...
                                    <p className="mt-2 font-semibold">{proposal.target_label}</p>
                                    <p className="mt-2 text-sm opacity-80">{proposal.created_at}</p>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {proposals.map((proposal) => {
                            const content = proposal.type === 'review_response'
                            ...
                            return (
                                <article
                                    ...
                                    className={`rounded-[2rem] border p-6 shadow-sm transition ${
                                        isEditing ? 'border-slate-900 bg-white' : 'border-slate-200 bg-white'
                                    }`}
                                >
                                    ...
                                    {isEditing ? (
                                        <form
                                            ...
                                            onSubmit={(event) => {
                                                ...
                                                form.patch(admin.proposals.update({ proposal: proposal.id }).url, {
                                                    preserveScroll: true,
                                                    onSuccess: () => {
                                        ...
                                        >
                                            <label className="block text-sm font-medium text-slate-700">
                                                ...
                                                <textarea
                                                    ...
                                                    className="mt-2 min-h-32 w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-slate-900"
                                                />
                                            </label>
                                            <label className="block text-sm font-medium text-slate-700">
                                                Rationale
                                                <textarea
                                                    ...
                                                    className="mt-2 min-h-24 w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-slate-900"
                                                />
                                            </label>
                                            <label className="block text-sm font-medium text-slate-700">
                                                Confidence
                                                <input
                                                    ...
                                                    onChange={(event) => form.setData('confidence', Number(event.target.value))}
                                        ...
                                        <>
                                            ...
                                            <div className="mt-4 rounded-2xl bg-slate-50 p-4">
                                                <p className="font-medium text-slate-900">
                                                    {proposal.type === 'review_response' ? 'Suggested response' : 'Suggested change'}
                                                </p>
